...
export default function TimelineCard({ entry, onClick }: TimelineCardProps) {
  ...
  return (
    <div
      onClick={onClick}
      className="border border-gray-200 rounded-lg p-3 md:p-4 mb-3 md:mb-4 cursor-pointer hover:shadow-lg transition-shadow active:opacity-90 h-full flex flex-col"
      style={{ background: 'var(--card-background)' }}
    >
      <div className="text-xs md:text-sm mb-1 md:mb-2" style={{ color: 'var(--foreground)', opacity: 0.7 }}>
        {entry.date && formatDate(entry.date)}
      </div>
      <h3 className="text-base md:text-lg font-semibold mb-1 md:mb-2 line-clamp-2" style={{ color: 'var(--foreground)' }}>{entry.title}</h3>
      <p className="text-sm md:text-base mb-2 md:mb-3 line-clamp-3 flex-grow" style={{ color: 'var(--foreground)', opacity: 0.85 }}>{entry.excerpt}</p>
